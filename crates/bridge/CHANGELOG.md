...
- Made process teardown both **recycle-safe** and **non-blocking**: the process tree is killed (`killpg`) only while the child is still alive (`stop`/drop-before-exit), never after it has been reaped — once reaped the pid/group may be recycled, so signalling it could hit an unrelated process. `Drop` then **detaches** the stdout reader thread instead of joining it, so a descendant that inherited and kept stdout open after the direct CLI exited can never leave `Drop` blocked on `reader.join()`; the reader self-terminates on stdout EOF or when its next send fails as the channel is dropped.
