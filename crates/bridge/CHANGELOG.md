...
- Agent discovery (§3f-bis) — folder + scope + dedupe rework (operator-decided):
  ...
  - **User-global scope:** in addition to the per-workspace **project** folders (`<root>/.claude/agents`, `<root>/.copilot/agents`), the **user-global** folders `~/.claude/agents` and `~/.copilot/agents` are now scanned (home resolved dependency-free via `HOME` then `USERPROFILE`; injectable on the runtime as `with_global_home`, defaulting to the environment, `None` to disable). The global scope is the user's own home config — read **once**, independent of how many workspaces are scanned, and not gated by the workspace allowlist — and stays metadata-only with the 64 KiB cap and canonical-path **symlink containment against the home tree**.
  ...
  - `BridgeRuntime::discover_agents` now collects raw candidates (project roots + the global scope), filters them **per backend** against the backend allowlist (a backend is dropped from an entry if it can't be launched; an entry is dropped only if no backend remains), then merges by name. New module API: `discover_raw_in_root`, `discover_raw_global_in`, `merge_agents`, `user_home`, plus `AgentScope`/`AgentBackendBinding`/`RawAgent`/`GLOBAL_LABEL`.
