...
- Added `ChildRun::drain_remaining(timeout)`: a bounded, timeout-aware (`recv_timeout`) final drain of the stdout the child flushed on its way out, so the closing `result`/usage line (exact tokens + USD) is never lost to the channel being dropped on retirement. `stream()` retires the run under the lock (capturing the early-set vendor session id) but performs this drain — and the child drop that joins the reader — **on the owned, retired child after releasing the lock**, so neither the bounded drain nor the join ever blocks another run's `stream`/`reply`/`stop`.
